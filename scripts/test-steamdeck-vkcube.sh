...
printf 'RUN impl=%s display=%s xauthority=%s mode=%s layer=%s count=%s present_mode=%s extra=%s target_fps=%s min_generated=%s max_generated=%s multi_swapchain_cap=%s bfi_period=%s bfi_hold_ms=%s visual_hold_ms=%s benchmark=%s benchmark_label=%s\n' "${PPFG_LAYER_IMPL}" "\$DISPLAY" "\$XAUTHORITY" "\$PPFG_LAYER_MODE" "\$VK_INSTANCE_LAYERS" "${VKCUBE_COUNT}" "${VKCUBE_PRESENT_MODE:-default}" "${VKCUBE_EXTRA_ARGS:-none}" "${PPFG_ADAPTIVE_MULTI_TARGET_FPS:-default}" "${PPFG_ADAPTIVE_MULTI_MIN_GENERATED_FRAMES:-default}" "${PPFG_ADAPTIVE_MULTI_MAX_GENERATED_FRAMES:-default}" "${PPFG_MULTI_SWAPCHAIN_MAX_GENERATED_FRAMES:-default}" "${PPFG_BFI_PERIOD:-default}" "${PPFG_BFI_HOLD_MS:-default}" "${PPFG_VISUAL_HOLD_MS:-default}" "${PPFG_BENCHMARK:-default}" "${PPFG_BENCHMARK_LABEL:-default}"
